...
function leerArchivo() {
    fs.readFile('./data.json', 'utf-8', (error, datosLeidos) => {
        if (error) {
            console.error(error);
        }
        else {
...
function buscarAbilities(arreglo) {
...
function clasificarPorTypes(arregloTypes, arreglo) {
    const arregloRespuesta = [];
    arregloTypes
        .forEach((tipo) => {
        const arregloFiltrado = arreglo // electrico
            .filter((pokemon) => {
...
function clasificarPorAbilities(arregloTypes, arreglo) {
    const arregloRespuesta = [];
    arregloTypes
        .forEach((tipo) => {
        const arregloFiltrado = arreglo // electrico
...
function clasificarPorMove(arregloTypes, arreglo) {
    const arregloRespuesta = [];
    arregloTypes
        .forEach((tipo) => {
        const arregloFiltrado = arreglo // electrico
...
function filtrarPorAbecedario(arregloAbecedario, arreglo) {
    return arregloAbecedario
        .map((letra) => {
        const existeAlguno = arreglo.some((pokemon) => {
...
function buscarTodosLosAtributos(arregloStats, arreglo) {
    const respuesta = {};
    arregloStats
        .forEach((stat) => {
        const totalStatNumero = arreglo
            .reduce((valorAcumulado, pokemon) => {
            const statNecesitado = pokemon.stats // Speed
                .find((statHijos) => {
...
function tienenHeldItems(arreglo) {
